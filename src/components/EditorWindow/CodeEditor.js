  ...
  return (
    ...
    <div className="overlay rounded-md w-full h-full shadow-4xl">
      ...
      {/* <button onClick={() => props.toggleDisplay(props.language)}>{tagState ? 'Back to Editing' : 'Command History'}</button> */}
        <button className="command-history-button" id={props.language} type="button" onClick={() => props.toggleDisplay(props.language)} onKeyDown={(e)=>{e.which === 13 && e.preventDefault()}}>{tagState ? 'Back to Editing' : 'Command History'}</button>
      </div>
      <Editor
